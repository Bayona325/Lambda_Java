...
public class Main {
    public static void main(String[] args) {
        ...
        var femeninos = campers.stream()
            .filter((camper) -> camper.getNombre() == Sexo.FEMENINO)
            .map((c) -> 1)
            .reduce(0, (acomulador, item) -> acomulador + item);
        var masculinos = campers.stream()
            .filter((camper) -> camper.getNombre() == Sexo.MASCULINO)
            .map((c) -> 1)
            .reduce(0, (acomulador, item) -> acomulador + item);
        var otros = campers.stream()
            .filter((camper) -> camper.getNombre() == Sexo.OTRO)
            .map((c) -> 1)
            .reduce(0, (acomulador, item) -> acomulador + item);

        System.out.println("Total: " + femeninos);
        System.out.println();
        System.out.println();
        System.out.println();
        */
